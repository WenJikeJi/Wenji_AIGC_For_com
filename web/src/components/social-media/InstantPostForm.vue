<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-lg max-w-3xl w-full mx-4 max-h-[90vh] overflow-y-auto">
      <div class="p-6">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">即时发布内容</h2>
          ...
          <div class="flex items-center space-x-4">
            ...
            <label class="inline-flex items-center">
              <input 
                ...
                class="text-blue-600 focus:ring-blue-500"
              />
              <span class="ml-2">定时发布</span>
            </label>
          </div>
          
          <!-- 定时发布时间选择 -->
          <div v-if="publishTime === 'scheduled'" class="mt-4">
            <div class="relative">
              <input 
                type="datetime-local"
                v-model="scheduledDateTime"
                class="w-full px-4 py-3 text-sm rounded-xl bg-white border-2 border-gray-200 focus:outline-none focus:border-transparent focus:ring-2 focus:ring-purple-500 transition-all duration-300 shadow-sm hover:shadow-md"
                :min="minDateTime"
              />
              <div class="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500 to-indigo-600 opacity-0 hover:opacity-5 transition-opacity duration-300 pointer-events-none"></div>
            </div>
          </div>
        </div>
  ...
  setup() {
    ...
    const isFormValid = computed(() => {
      return selectedPlatforms.value.length > 0 && 
             ...
             (!isScheduled || scheduledDateTime.value)
    })
    
    // 是否为定时发布
    const isScheduled = computed(() => publishTime.value === 'scheduled')
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const postData = {
          ...
          scheduledDateTime: isScheduled.value ? scheduledDateTime.value : null,
        ...
        // 发送发布请求
        const response = await socialMediaAPI.publishInstantPost(postData)
